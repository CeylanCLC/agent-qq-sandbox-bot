...
import httpx
import websockets
from project_config import BOT_DISPLAY_NAME, BOT_PERSONA_NAME, BOT_PERSONA, BOT_TRIGGER_WORDS


ONEBOT_WS = os.getenv("ONEBOT_WS", "ws://127.0.0.1:3001").strip()
OPENCLAW_URL = os.getenv("OPENCLAW_URL", "http://127.0.0.1:29306/v1/chat/completions").strip()

HOST_SHARE_DIR = os.getenv("HOST_SHARE_DIR", "/root/openclaw-qq/share")
CONTAINER_SHARE_DIR = os.getenv("CONTAINER_SHARE_DIR", "/share")
...
# 群聊关键词触发
TRIGGER_WORDS = sorted(BOT_TRIGGER_WORDS)
...
ACTIVE_GROUP_CONTEXT_LIMIT = int(os.getenv("ACTIVE_GROUP_CONTEXT_LIMIT", "15"))

# 每隔多久巡群一次：3小时
PERIODIC_REVIEW_INTERVAL_SECONDS = int(os.getenv("PERIODIC_REVIEW_INTERVAL_SECONDS", str(3 * 60 * 60)))
...
BLOCKED_GROUP_IDS = ceylan_runtime.BLOCKED
PERIODIC_REVIEW_USE_IMAGES = os.getenv("PERIODIC_REVIEW_USE_IMAGES", "0").strip() == "1"

# 每轮巡群最多处理多少个群，防止一下子太耗 token
MAX_GROUPS_PER_REVIEW = 20

# 巡群状态落盘文件：保存缓存、last_seq、已发过的巡群回复，防止重启后失忆
PERIODIC_STATE_FILE = Path(os.getenv("PERIODIC_STATE_FILE", "/root/openclaw-qq/periodic_state.json"))

# 群长期压缩记忆文件：不保存全部历史原文，只保存每个群的长期摘要
GROUP_MEMORY_STATE_FILE = Path(os.getenv("GROUP_MEMORY_STATE_FILE", "/root/openclaw-qq/group_memory_state.json"))
...
def format_messages_for_memory(group_id, since_seq=0, limit=140):
    ...
    for m in msgs:
        ...
        name = BOT_DISPLAY_NAME if is_bot else (m.get("nickname") or m.get("user_id") or "群友")
...
async def compact_group_memory_once(group_id, force=False):
    ...
    prompt = f"""
你是QQ群机器人“{BOT_DISPLAY_NAME}”的长期记忆压缩器。
...
async def periodic_group_memory_compaction():
    ...
    while True:
        try:
            ...
            group_ids = [gid for gid in group_ids if gid not in BLOCKED_GROUP_IDS]
...
def load_bootstrap_prompt():
    try:
        text = Path(os.getenv("OPENCLAW_BOOTSTRAP_FILE", "/root/.openclaw/workspace/BOOTSTRAP.md")).read_text(encoding="utf-8").strip()
        if text:
            return text
    except Exception:
        pass
    ...
    return f"""
{BOT_PERSONA}
你的显示名称是“{BOT_DISPLAY_NAME}”，人物名称是“{BOT_PERSONA_NAME}”。
...
def get_openclaw_token():
    ...
    cfg = json.loads(Path(os.getenv("CEYLAN_OPENCLAW_CONFIG", "/root/.openclaw/openclaw.json")).read_text())
...
VISION_ENABLE_THINKING = os.getenv("VISION_ENABLE_THINKING", "0").strip() == "1"
...
def extract_task_text(text):
    ...
    names = "|".join(re.escape(x) for x in {BOT_DISPLAY_NAME, BOT_PERSONA_NAME} if x)
    prefix = rf"^(?:(?:{names})\s*)?" if names else "^"
    patterns = [
        prefix + r"(?:任务|执行任务|agent任务|Agent任务)\s*[:：,，]?\s*(.+)$",
        r"^(?:/任务|#任务)\s*[:：,，]?\s*(.+)$",
    ]
...
def parse_send_asset_command(text):
    ...
    names = "|".join(re.escape(x) for x in {BOT_DISPLAY_NAME, BOT_PERSONA_NAME} if x)
    prefix = rf"^(?:(?:{names})\s*)?" if names else "^"
    m = re.match(prefix + r"发图\s*[:：]\s*(.+)$", t)
    if m:
        return "image", m.group(1).strip()

    m = re.match(prefix + r"发文件\s*[:：]\s*(.+)$", t)
    if m:
...
def record_group_message(event, text, image_urls=None):
    ...
    # every 20 human messages persist state
    if group_message_seq[group_id] % 20 == 0:
        save_periodic_state()
...
def build_group_recent_text(group_id, limit=80, only_new=False):
    ...
    for m in msgs:
        ...
        name = BOT_DISPLAY_NAME if is_bot else (m.get("nickname") or m.get("user_id"))
        text = m.get("text", "")
        ts = m.get("time", "")
...
async def call_mimo_vision(messages, max_tokens=800, timeout=300):
    ...
    # Qwen/SiliconFlow compat: merge all system messages into one first system message.
    system_parts = []
    non_system_messages = []
    for msg in messages:
...
@ceylan_runtime.guard('')
@ceylan_runtime.serial_history
async def ask_openclaw(user_key, text, group_id=None, image_urls=None):
    ...
    user_text = (
        f"请严格按照“{BOT_DISPLAY_NAME} / {BOT_PERSONA_NAME}”的人物设定回答。"
    ...
    try:
        ...
        for step in range(1, MAX_TASK_STEPS + 1):
            try:
                ...
                await send_group_msg(ws, group_id, user_id, "任务暂时中断，请稍后再试。", mention=False)
                return
        ...
        try:
            ...
            for group_id in group_ids:
                try:
                    ...
                    if not PERIODIC_REVIEW_USE_IMAGES:
                        periodic_image_urls = []
...
你要基于“长期压缩记忆 + 新增消息”判断是否值得作为“{BOT_DISPLAY_NAME} / {BOT_PERSONA_NAME}”主动接一句。
...
async def handle_group_message(ws, event):
    ...
    if task_text:
        ...
        ceylan_runtime.spawn_task(run_agent_task(ws, group_id, user_id, task_text, mention=at_me))
        return
    ...
    try:
        ...
        print("[模型请求失败] " + type(e).__name__, flush=True)
        answer = "暂时连接不上模型，请稍后再试。"
...
async def main():
    await ceylan_runtime.run_bridge(globals())
